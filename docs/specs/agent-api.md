...
> Статус: `draft` · обновлено 2026-09-19 · ответственный: Анатолий + Claude Code
...
Секрет триггера (с 19.09): хранится в таблице `system_settings`
(`key='cron_secret'`, jsonb-строка) — единый источник и для pg_cron-задачи
(она читает его из таблицы в момент пинга), и для функции. Запасной канал —
env `CRON_SECRET` в Vercel (его Vercel Cron подставляет сам в ежедневный догон).
История: до 19.09 секрет жил только в env и в pg_cron-задаче осталась заглушка
из миграции — все 15-минутные вызовы получали 401, публикация молча
деградировала до ежедневного резерва (урок: секреты «в двух местах» не живут).

Переменные окружения (Vercel → Settings → Environment Variables):
`CRON_SECRET` (запасной, для Vercel Cron-догона), `TG_BOT_TOKEN`,
`TG_CHANNEL_ID` (по умолчанию `@rightway_platform`). Пока `TG_BOT_TOKEN` не
настроен, очередь не трогается — посты остаются `pending`.
