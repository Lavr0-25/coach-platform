...
> Статус: `stable` · обновлено 2026-09-12 · ответственный: Анатолий + Claude Code
...
- **Мат-фильтр на сервере** (миграция
  ...
  дружелюбный первый барьер, серверная проверка — обязательная: до миграции
  обращение с матом можно было отправить напрямую через REST, минуя UI.
