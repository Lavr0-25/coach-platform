...
export default function LessonComments({ lessonId, courseId }: LessonCommentsProps) {
  ...
  const targetType = lessonId ? 'lesson' : 'course'
  ...
  const handleSubmit = async (e: React.FormEvent, parentId: string | null = null) => {
    ...
    try {
      ...
      // Добавляем рейтинг только для главных комментариев (не ответов)
      if (!parentId && newRating !== null) {
        // Проверяем, есть ли уже отзыв с рейтингом
        if (userHasReview) {
  ...
  const handleUpdateComment = async (commentId: string) => {
    ...
    try {
      ...
      // Если это главный комментарий, можно обновить рейтинг
          ...
          >
            <svg
              className={`${sizeClasses[size]} ${
                rating !== null && star <= rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'
              }`}
              viewBox="0 0 20 20"
            >
  ...
  const filteredComments = showReviewsOnly 
    ? comments.filter(c => c.rating !== null)
    : comments

  const reviewsCount = comments.filter(c => c.rating !== null).length
  const averageRating = reviewsCount > 0 
    ? comments.filter(c => c.rating !== null).reduce((sum, c) => sum + (c.rating || 0), 0) / reviewsCount
    : 0
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-purple-100 p-4 sm:p-6">
      {/* Заголовок с рейтингом и фильтром */}
        ...
        isBanned ? (
          ...
          <div className="mb-6 p-4 sm:p-5 bg-gradient-to-br from-purple-50 to-blue-50 rounded-xl border border-purple-100">
            ...
            <form onSubmit={(e) => handleSubmit(e, null)} className="space-y-4">
              {/* Звёзды для отзыва (только если ещё нет отзыва) */}
              {!userHasReview && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Оценка (необязательно, только один раз)
                  </label>
                  {renderStars(newRating, true, 'lg')}
                  {newRating !== null && (
                    <p className="text-xs text-purple-600 mt-1">
                       Вы ставите оценку {newRating}/5. Это можно сделать только один раз.
                    </p>
                  )}
                </div>
              )}
              ...
              {userHasReview && (
                <div className="bg-purple-100 border border-purple-200 rounded-lg p-3 text-sm text-purple-700">
                  ...
                  <p className="text-xs mt-1">
                    Вы можете отредактировать свой отзыв в списке ниже.
                  </p>
                </div>
              )}
        ...
        {filteredComments.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm sm:text-base">
              {showReviewsOnly ? 'Пока нет отзывов с оценками' : 'Пока нет комментариев. Будьте первым!'}
            </p>
          </div>
        ) : (
            ...
            return (
              <div 
                ...
                className={`border-b border-purple-100 pb-6 last:border-0 transition-all duration-500 ${
                  comment.rating !== null ? 'bg-purple-50/30 rounded-xl p-4 border border-purple-200' : ''
                }`}
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {editingCommentId === comment.id ? (
                      <div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-xl">
                        {/* Редактирование рейтинга только для главных комментариев */}
                        {!comment.parent_id && (
                          <div className="mb-3">
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Оценка:
                            ...
                            <div className="flex gap-0.5">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <button
                                  key={star}
                                  type="button"
                                  onClick={() => setEditRating(star === editRating ? null : star)}
                                  className="cursor-pointer hover:scale-110 transition-transform focus:outline-none"
                                >
                                  <svg
                                    className={`w-6 h-6 ${
                                      editRating !== null && star <= editRating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'
                                    }`}
                                    viewBox="0 0 20 20"
                                  >
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
