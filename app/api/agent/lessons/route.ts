...
// POST  /api/agent/lessons — создать черновик текстового урока: { topic_id?, title, description?, html }
// PATCH /api/agent/lessons — опубликовать черновик: { id, publish: true }
//
...
export async function POST(request: Request) {
  ...
  // Тема указана → привязываем урок и переводим тему в работу
  if (topicId) {
    const { error } = await auth.client
      .from('lesson_topics')
      .update({ lesson_id: lesson.id, status: 'in_progress', updated_at: new Date().toISOString() })
      .eq('id', topicId)
    if (error) {
  ...
  return Response.json({ ok: true, lesson, published: false }, { status: 201 })
}
