...
export async function startPurchase(formData: FormData): Promise<StartPurchaseResult> {
  ...
  //    user_id берём здесь же — он нужен расчёту комиссии (п. 4).
  const { data: coach } = await admin
    .from('coaches')
    .select('paid_publishing_allowed, user_id')
    .eq('id', item.coach_id)
    .maybeSingle()
  if (!coach?.paid_publishing_allowed || !coach.user_id) {
  ...
  // 4. Деньги: комиссия из настроек (ручная ставка → глобальная − бенефиты),
  //    роялти автору — остаток
  const commission = await getEffectiveCommission(admin, coach.user_id)
